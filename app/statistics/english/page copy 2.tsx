...
export default function EnglishStatistics() {
	...
	const allUsersStatisticsList = useSelector(
	 ...
	 const currentUsersStatisticsList = useFilteredEnglishStatistics(
...
useEffect(() => {
	...
	if (currentUsersStatisticsList.length === 0) {
		return <Loading />;
	}
	...
	return (
		<div className='container mx-auto px-4 p-8 flex flex-col space-y-6 max-w-screen-sm items-center'>
			...
			<div>
				{currentUsersStatisticsList.length > 0 ? (
					currentUsersStatisticsList.map(el => (
						<pre
							key={el.createdAt.toString()}
							className='bg-gray-200 p-2 rounded'
						>
							<div>Тест № {currentUsersStatisticsList.indexOf(el) + 1}</div>
