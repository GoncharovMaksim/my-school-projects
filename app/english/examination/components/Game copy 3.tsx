...
export default function Game({ gameSettings, setGameSettings }: GameProps) {
	...
	function shuffleArray(array) {
		...
		//console.log('shuffledArray', shuffledArray);// Создаем копию массива
	...
	//console.log(gameSettings.examWordsList[2]?.englishWord);
	// useEffect(() => {
	// 	arrRandomWords;
	// }, [arrRandomWords]);
	// useEffect(() => {
	...
	useEffect(() => {
		if (arrRandomWords.length > 0) {
			const currentWord = arrRandomWords[gameSettings.stepGame]; // Берем первый элемент после обновления
	...
	function startGame() {
		...
		//console.log(arrRandomWords);
		const minNumber = 0;
		const maxNumber = gameSettings.examWordsList.length;
		...
		//setQuestion(arrRandomWords[gameSettings.stepGame].englishWord);
		// console.log(
		// 	'shuffledWords[gameSettings.stepGame].englishWord',shuffledWords[
		// 		gameSettings.stepGame
		// 	].englishWord
		// );
