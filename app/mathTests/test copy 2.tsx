...
export default function Test({
  ...
  useEffect(() => {
    ...
    getQuestions(userCheckOptions).then(response => {
      ...
      // перемешаем опции случайным образом

      for (let i = randomQuestions.length - 1; i > 0; i--) {
        const tempQuestion = randomQuestions[i];

        for (let j = tempQuestion.options.length - 1; j > 0; j--) {
          const random = Math.floor(Math.random() * (j + 1));

          [tempQuestion.options[j], tempQuestion.options[random]] = [
            tempQuestion.options[random],
            tempQuestion.options[j],
          ];
        }
      }
